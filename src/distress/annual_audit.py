...
def audit(raw, parsed):
    ...
    cover='\n'.join(p['text'] for p in raw['pages'][:6])
    ...
    return {'method':VERSION,'verification_tier':'rule_based_source_checks_not_manual_review',
            ...
            'row_evidence':[r for r in parsed['rows'] if r['metric'] in NEEDED]}
...
def run(root):
    ...
    for did,expected in sorted(docs.items()):
        ...
        p=audit(raw,parse(raw));p['extraction_sha256']=digest(path)
