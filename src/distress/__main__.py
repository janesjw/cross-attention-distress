...
def main():
    p=argparse.ArgumentParser(description='Public disclosure reconstruction database')
    sub=p.add_subparsers(dest='command',required=True)
    a=sub.add_parser('audit');a.add_argument('--database',default=str(ROOT/'data/reconstruction.sqlite'))
    a=sub.add_parser('features');a.add_argument('--database',default=str(ROOT/'data/reconstruction.sqlite'));a.add_argument('--firm',required=True);a.add_argument('--year',required=True,type=int)
    ...
    a=sub.add_parser('extract-section');a.add_argument('--database',default=str(ROOT/'data/reconstruction.sqlite'));a.add_argument('--document',required=True);a.add_argument('--name',required=True);a.add_argument('--start-page',type=int,required=True);a.add_argument('--end-page',type=int,required=True)
    a=sub.add_parser('export');a.add_argument('--database',default=str(ROOT/'data/reconstruction.sqlite'));a.add_argument('--dataset-id',required=True);a.add_argument('--output',required=True)
    args=p.parse_args()
    if args.command=='download-sources':
        from .acquire import download_verified
        result=download_verified(json.loads((ROOT/'data/verified_seed.json').read_text())['documents'],ROOT)
