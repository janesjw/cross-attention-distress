...
`Research data pipeline` runs every six hours and can also be started from Actions → Run workflow. Each run attempts up to 25 reports with a 25-minute processing budget. It reads the complete saved filing index, skips intact completed extractions, and retries failed reports on up to three runs.
